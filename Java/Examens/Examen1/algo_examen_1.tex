...
\emph{Attention: il peut y avoir plusieurs bonnes réponses par question.}
...
\begin{enumerate}
  ...
  \item Quelles commandes lancent l'exécution d'un programme \java?
  \begin{itemize}
    \boxitem \texttt{javac prog}
    \boxitem \texttt{java prog.class}
...
double[][] values =
  ...
  \item Quelle est la valeur retournée par \verb+things.length+?
  \begin{verbatim}
double[][] things =
  ...
  7~colonnes et l'affecte à \verb+stuff+?
  \begin{itemize}
...
int[][] items = {{0,1,3,4},{4,4,99,0,7},{3,2}};
  \end{verbatim}
  \vspace*{-10pt}
  Quelles boucles affichent tous les éléments de \verb+items+?
  \begin{itemize}
    \boxitem
      \begin{verbatim}
...
for (int i = z.length - 1; i >= 0; i--) {
  ...
  \item Écrivez, une fois avec une boucle \texttt{for}, une fois
  avec une boucle \texttt{while}, le code qui calcule et affiche la
  valeur de $4^5$.

\pagebreak

  \item Écrire un programme \texttt{ChangeSousChaine} qui permet de
  substituer une sous-chaîne par une autre dans une chaîne saisie par
