...
\texttt{Makefile}). Ã vous de construire l'évaluateur.
...
\begin{itemize}
  ...
  \item \texttt{parser.mly}: le spécification de la syntaxe concrète
    ...
    \texttt{parser.mli} (interface spécifiant le type des lexèmes et
    le type de l'analyseur syntaxique) et \texttt{parser.ml},
    contenant l'analyseur syntaxique proprement dit.
...
\begin{enumerate}
  ...
  \item l'analyseur syntaxique \textsf{Parser} exporte une fonction
    ...
    l'entrée standard, et renvoie l'arbre de syntaxe abstraite qui
    doit être évalué;
  ...
  \item le pilote \textsf{Main} invite l'usager à saisir une
    ...
    (dans un environnement vide); il affiche ensuite le résultat. En
    cas d'erreur, il affiche l'erreur dont le message est associé aux
