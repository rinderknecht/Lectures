...
\DefTirName{add} (p.~\pageref{semantique_operationnelle_etendue_lisible}):
{\small
 \begin{mathpar}
    \inferrule*
      {\eval
...
\DefTirName{let} (p.~\pageref{semantique_operationnelle_etendue_lisible}):
{\small
\begin{mathpar}
  \inferrule
     {\eval
...
\bigskip
En conjonction avec \DefTirName{abs-dyn}, cela implique alors que
...
avec notre sémantique \DefTirName{abs-dyn}, la valeur d'une variable
...
En observant les règles \DefTirName{let}, d'une part, et
\DefTirName{abs} et \DefTirName{app} d'autre part, on peut se rendre
compte que la règle \DefTirName{let} peut être supprimée sans
...
La règle \DefTirName{app} (p.~\pageref{abs-app}) peut se réécrire en
...
\end{mathpar}
Or l'axiome \DefTirName{abs} dit
$\inferrule
  {\meval{\rho}
...
La règle \DefTirName{let} ajoute une nouvelle liaison dans
