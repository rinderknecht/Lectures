...
\begin{itemize}
  ...
  \item The determinism means that, for any expression, there is only
    one rule to rewrite it. This implies that, every time a program is
    run with the same input, the output will be the same. This is a
...
Anyway, in the presence of infinite reductions, the soundness property
...
extend our boolean expressions with a \textbf{conditional expression}:
\begin{itemize}
...
%
\begin{frame}
\frametitle{Conditional expressions (cont)}

It is easy to extend the rewrite system to cope with conditional:
\begin{mathpar}
\inferrule
{}
...
reduced \emph{before} the others. This is the usual way in programming
languages, the rationale being to avoid unnecessary computations
(evaluating \(e_2\) and \(e_3\) is not necessary).
