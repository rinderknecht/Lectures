...
handling: we just use the magic keyword \kw{error}. This is because we
leave for the program to detail what to do and maybe use some specific
features of its language, like exceptions.
...
(\id{e},\id{q_1}), \id{e_1})\) \RComment Rule \(\rightarrow_2\)
\zi    \End
\zi  \End
\end{codebox}
Termination ensues from \((\id{e}, \id{q}) =
