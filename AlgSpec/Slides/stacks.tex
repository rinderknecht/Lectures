...
\proc{Append} by noting that if one of the stacks is empty, then the
...
Let us call \textbf{terms} the objects constructed using the functions
of the specification, and \textbf{variables} ranging over terms, e.g.,
\proc{Empty} and \(\proc{Push} (\id{e}, \proc{Empty})\), where \(e\)
is a variable.
...
Let us underline both calls to \proc{Append}:
\[
\underline{\proc{Append} (\proc{Push} (\id{e}, \id{s_1}), \id{s_2})}
= \proc{Push} (\id{e}, \underline{\proc{Append} (\id{s_1}, \id{s_2})})
\]
Let us now underline only the differences between the two:
\[
\proc{Append} (\underline{\proc{Push} (\id{e}, \id{s_1})}, \id{s_2})
= \proc{Push} (\id{e}, \proc{Append} (\underline{\id{s_1}}, \id{s_2}))
\]
