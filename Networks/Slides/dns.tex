...
%
\begin{frame}
\frametitle{Internet Directory Service (DNS)}
...
\textbf{Domain Name Service} (\textbf{DNS}).
...
%
\begin{frame}
\frametitle{Internet Directory Service (DNS) (cont)}
...
The DNS protocol runs over \textsc{udp} and uses port 53.

DNS is commonly used by other application-layer protocols, like
...
\begin{enumerate}
  ...
  \item passes it to the client side of the DNS application,

  \item this application sends to a DNS server the hostname,
...
\begin{itemize}
  ...
  DNS queries (from \textsc{http} requests and e-mails at
...
%
\begin{frame}
\frametitle{DNS/Local name servers}
...
%
\begin{frame}
\frametitle{DNS/Root name servers}

When a local name server can not serve a request, it then acts as a
DNS client and queries a \textbf{root name server}.
...
%
\begin{frame}
\frametitle{DNS/Authoritative name
  servers}
...
back to host \url{surf.eurocom.fr}, through all the DNS
...
%
\begin{frame}
\frametitle{DNS/Recursive
  and iterative queries}

As in our example, when a name server \emph{A} becomes a DNS
...
Each DNS message (either query or response) carries one or
...
\begin{itemize}
  ...
  kind of record is used to route DNS queries along in the
...
%
\begin{frame}
\frametitle{DNS/Messages}

\vspace*{-1pt}

There are only two kind of DNS messages and both queries and
