...
%\begin{center}
\centerline{\textbf{Durée: 2 heures. Documents et calculatrices ne sont
pas autorisés.}}
%\end{center}
...
\begin{enumerate}
  ...
  \item construisez l'arbre correspondant (\textbf{à l'encre}),

  \item dans chaque arbre, reliez les variables dans les expressions à
  leur lieur (\textbf{dans une encre différente}),

  \item identifiez les variables libres (\textbf{à l'encre dans
  l'arbre et dans une phrase}),

  \item décrivez formellement son exécution à l'aide de la sémantique
  suivante, où $\evalf{e}{\rho}$ est la valeur de l'expression $e$
...
\evalf{e_1 \,\, e_2}{\rho} &= \evalf{e}{((x \mapsto \evalf{e_2}{\rho})
  \oplus \rho')} \ \ \textnormal{où} \ \ \evalf{e_1}{\rho} = \langle\Xfun \,\, x
\rightarrow e,\rho'\rangle
\end{align*}
  L'évaluation consiste à appliquer les équations de la gauche vers la
  droite jusqu'au résultat ou une impossibilité (c.-à-d. erreur à
  l'exécution).
...
\noindent Le type des arbres binaires est défini par:
\begin{verbatim}
...
\begin{itemize}
  ...
  \bitem toutes les étiquettes du sous-arbre de gauche
  (respectivement, de droite) sont inférieures (respectivement,
  supérieures) à l'étiquette de la racine,

  \bitem récursivement, les deux sous-arbres sont eux-mêmes des arbres
  de recherche.
...
\begin{itemize}
  ...
  \bitem Écrire une fonction \texttt{minimum} qui renvoie le plus
  petit élément d'un arbre binaire de recherche. On déclenchera
  l'exception \texttt{Not\_found} si l'arbre est vide.

  \bitem Écrire une fonction \texttt{appartient} currifyée qui indique
  si une étiquette donnée apparaît dans un arbre de recherche
  donné. On tirera parti du fait que l'arbre est classé pour que la
  recherche soit plus rapide que dans un arbre binaire quelconque.

  \bitem Écrire une fonction \texttt{ajoute} qui ajoute un n{\oe}ud
  dans un arbre de recherche de façon à ce que celui-ci reste un arbre
  de recherche. Cela se fait en rajoutant une feuille à l'arbre.
