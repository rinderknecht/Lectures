...
\emph{Un arbre binaire est un arbre où chaque nœud a au plus deux
fils, un fils gauche et un fils droit. De plus, chaque nœud est annoté
par une étiquette, qui est une valeur a priori quelconque. Voici un
...
type {\tt int arbre} lorsque l'on veut annoter chaque nœud par un
entier, etc.}
\begin{itemize}

  \item \emph{Écrire une fonction qui calcule la hauteur d'un arbre.}
...
\end{verbatim}
    ...
    La fonction \texttt{max} est polymorphe et prédéfinie, mais dans
    le cas précis (elle s'applique à des entiers naturels) elle
    pourrait être simplement définie par
...
\end{verbatim}
    ...
    \item \emph{Écrire une fonction qui calcule la taille d'un arbre
    (c'est-à-dire le nom\-bre de nœuds).}
...
\begin{center}
  \includegraphics[scale=0.8]{arech.eps}
\end{center}
