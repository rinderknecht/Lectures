...
\texttt{x}. Or l'opérateur plus \texttt{(+)} a pour type $\texttt{int}
...
\item \emph{On désire manipuler symboliquement des expressions formées
      ...
      \begin{itemize}
        ...
        ces expressions (les variables peuvent être représentées par
        des chaînes de caractères);}
              ...
              (suite de Â«~ou~Â»). On définit donc un constructeur pour
...
let rec ajout p1 p2 =
  ...
  \item \emph{Un arbre binaire est un arbre où chaque nœud a au plus
  deux fils, un fils gauche et un fils droit. De plus, chaque nœud est
...
\end{verbatim}
         ...
         \item \emph{Écrire une fonction qui calcule la taille d'un
               arbre (c'est-à-dire le nom\-bre de nœuds).}
