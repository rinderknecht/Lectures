...
indication si la lettre de l'arc entrant est une fin de mot et la
suite des mots partageant le même début. La figure suivante montre le
\emph{trie} des mots «~le~», «~les~», «~la~» et «~son~», l'astérisque
marquant la fin d'un mot:}
...
let select lettre trie =
  ...
  qui vérifie si une chaîne de caractères est un mot dans un {\em
...
| mot::reste ->
  ...
  \item[\textbf{A.6}] \emph{On désire écrire une nouvelle fonction de
       recherche qui donne une signification spéciale au caractère
       point (\texttt{.}). Ce caractère se comportera comme un
       \emph{joker} pouvant remplacer n'importe quelle lettre. Écrire
       une fonction qui prend en entrée un lexique et un mot pouvant
...
\begin{itemize}
  ...
  \item[\textbf{B.1}] \emph{Écrire une fonction
...
let singletons l = List.map (fun x -> [x]) l
  ...
  \item[\textbf{B.2}] \emph{Écrire une fonction {\tt merge} qui
...
let rec merge ordre l1 l2 =
  ...
  \item[\textbf{B.3}] \emph{Écrire une fonction {\tt merge2à2} qui
...
\begin{itemize}
  ...
  \item[\textbf{C.1}] \emph{Écrire une fonction {\tt partage} qui
