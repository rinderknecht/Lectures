...
\title{Corrigé du TP 2 de programmation fonctionnelle en Objective Caml}
...
\noindent On aurait pu écrire la dernière ligne:
\verb|iter (n-1) f (f x)|

\medskip

\noindent La version naïve de Fibonnacci est
\begin{verbatim}
let rec fib n = if n < 2 then 1 else fib (n-1) + fib (n-2);;
\end{verbatim}

\noindent Soit $A_n$ le nombre d'appels récursifs pour calculer
...
B_{n-2} $$
On prouve par récurrence sur $n$ que $\forall n \in \mathbb{N}, B_n =
2 F_n$. Par conséquent $A_n = 2 F_n - 1$. Le temps d'exécution de
...
efficacement (le nombre d'appels est linéaire en $n$ maintenant):

\begin{verbatim}
let fib n = fst (iter n (fun (x,y) -> (x+y,x)) (1,1));;
\end{verbatim}

\noindent La fonction \texttt{fst} est prédéfinie mais peut se définir
trivialement par:
\begin{verbatim}
let fst (x,y) = x;;
\end{verbatim}
...
la suite $(f^k(x), n-k)_{k \geq 0}$ et on s'arrête lorsque la deuxième
composante s'annule. On obtient donc la paire $(f^n(x), 0)$, dont on
extrait la première composante grâce à {\tt fst}. Cette fonction est
prédéfinie mais peut se définir trivialement, ainsi que \texttt{pred}
qui calcule le prédécesseur d'un entier par:
\begin{verbatim}
let fst (x,y) = x
let pred x = x - 1
\end{verbatim}
...
\noindent Opérations sur les matrices:
\begin{verbatim}
...
\noindent On ne peut pas utiliser un simple filtrage pour déterminer
si les quatre éléments sont égaux (cf. énoncé).
\begin{verbatim}
let trig_sup m =
  match m with
    (_, (0.0, _)) -> true
