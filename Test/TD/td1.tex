...
programme donné. Un graphe est un ensemble fini de nœuds et un
ensemble d'arcs les reliants, ces arcs pouvant être étiquetés. Chaque
nœud correspond à un point du programme (par exemple une
...
variables). Les arcs sont donc orientés. Parmi les nœuds, on
distingue aussi un nœud correspondant à l'état initial et un
nœud correspondant à l'état terminal (ou \emph{final}). Pour les
distinguer aisément, nous ajoutons deux arcs, l'un entrant vers le
nœud initial (arrivant de nulle-part) et l'autre sortant du
nœud terminal (allant nulle-part). Le graphe capture un
...
nombre de nœuds. Après le calcul du graphe de contrôle, la
...
d'associer un nœud du graphe à chaque instruction et expression
...
faut rajouter des nœuds de jonction correspondant à la fin d'une
...
\begin{itemize}
  ...
  \item \textbf{L'instruction simple.} À toute instruction de simple
    ...
    nœud est évidemment~1, donc le nombre cyclomatique de ce
    graphe est $\gamma = 0 - 1 + 2 = 1$. L'interprétation de ce nombre
    en termes de majorant du nombre minimal de chemins recouvrants (du
    nœud initial au nœud terminal) est trivialement vérifiée
    ici: il n'y a besoin que d'un seul chemin qui est le nœud $1$
    lui-même.
  ...
  \item \textbf{La séquence d'instructions.} Les instructions sont
    ...
    \end{center}
    \noindent
     S'il y a $n$ instructions, il y a donc $n$ nœuds. Étant donné
     ...
     d'un seul chemin pour couvrir tout le graphe du nœud initial
     au nœud terminal.
  ...
  \item \textbf{La conditionnelle.} Elle a pour syntaxe
    ...
    \textbf{endif}}. Les nœuds du graphe de con\-trô\-le
    correspondent à l'expression booléenne (nœud étiqueté~1), aux
    instructions (nœuds étiquetés~2 si la condition est vraie,
    et~3 sinon) et au mot-clé \textsf{\textbf{endif}} (nœud
    étiqueté~4). Nous avons le graphe de la conditionnelle complète
    \begin{center}
      \includegraphics[scale=0.7]{complete_cond.eps}
    \end{center}
    \noindent
    Le nombre d'arcs est~4, et le nombre de nœuds est~4
    aussi. Donc le nombre cyclomatique de ce graphe est $\gamma =
    2$. Il est évident qu'il suffit de deux chemins, $(1,2,4)$ et
    $(1,3,4)$, pour couvrir tout le graphe du nœud initial au
    nœud terminal.
    ...
    \end{center}
    \noindent
    Le nombre d'arcs est~3 et le nombre de nœuds est~3 aussi. Donc
  ...
  \item \textbf{L'itération non bornée.} L'itération non bornée a pour
    ...
    \textbf{do} <\emph{instructions}> \textbf{done}}. Les nœuds du
    graphe de contrôle correspondent à l'expression booléenne
    (nœud étiqueté~1), aux instructions (nœud étiqueté~2) et
    au mot-clé \textsf{\textbf{done}} (nœud étiqueté~3). Le graphe
    est alors
    \begin{center}
  ...
  \item \textbf{L'itération bornée.} L'itération bornée est un cas
    ...
    e_0$, en étiquetant par~2 le nœud correspondant à $i \leqslant
    e_1$, en étiquetant par~3 le nœud correspondant à $c$, en
    étiquetant par~4 le nœud correspondant à $i \leftarrow i + 1$
    et en étiquetant par~5 le nœud correspondant au mot-clé
    \textsf{\textbf{done}}, on obtient le graphe de contrôle
    \begin{center}
      \includegraphics[scale=0.7]{for.eps}
    \end{center}
    \noindent
    Le nombre d'arcs est~5 et le nombre de nœuds est~5, donc le
...
\subsection{Graphes réductibles}
  ...
  \end{center}
  \noindent
  où les deux nœuds sont des instructions simples. Ainsi une
  séquence d'instructions se réduit à un seul nœud, mais le graphe
  de l'itération bornée est n'est pas réductible à celui de
  l'itération non-bornée, car le nœud qui initialise l'index de la
